...
| 구분                | 의미                | 내 해석                    |
| ----------------- | ----------------- | ----------------------- |
| Orthogonalization | 교란을 걷어내는 수학적 장치   | “Y와 T의 순수한 관계를 다시 복원한다” |
| Double ML         | 두 개의 보조모델을 동시에 학습 | “잔차로부터 기울기를 찾는다”        |
| Cross-fitting     | fold 간 독립 학습      | “자기 자신의 편향을 방지한다”       |
| CATE              | 개별 X의 처치 효과       | “SKU 단위 최적 할인율의 기울기”    |
| Refutation        | 가짜 변수 검정          | “이 효과가 우연인지 아닌지를 증명”    |


- DML은 예측모델이 아니라, ‘예측이 잘못되더라도 방향은 맞는 모델’을 만드는 일이다.
- 실제 가격최적화 업무에서는 DML이 모든 문제를 해결해주진 않는다.
- 하지만 confounder를 통제하고, post-treatment 변수를 제거하며, 정책(policy)을 정량적으로 평가하려는 시도는 데이터사이언티스트로서 “인과적 사고”를 체계화하는 첫걸음이었다.
