...
> Qingsong Wen et al., IJCAI 2021

---

## 1. 서론: 시계열에서의 데이터 증강 필요성
딥러닝 기반 시계열 분석(예: 분류, 예측, 이상탐지)은 대규모 학습 데이터에 의존하지만, 실제 환경에서는 레이블이 부족한 경우가 많습니다.  
이 논문은 **시계열 데이터 전용 데이터 증강(Data Augmentation, DA)** 기법을 **체계적으로 정리한 최초의 종합 리뷰 논문**입니다.

> 핵심 문제의식:
> - 이미지·음성 분야에서 검증된 증강 기법이 시계열에는 직접 적용되기 어렵다.
> - 시계열의 **시간적 종속성(temporal dependency)** 과 **도메인 의존성(task-dependency)** 을 고려한 전용 증강 전략이 필요하다.

---

## 2. 기본적 데이터 증강 방법 (Basic Methods)
...
- **Amplitude Perturbation (A(ω))**
- **Phase Perturbation (θ(ω))**
...
### 2.3 시간-주파수 영역 (Time-Frequency Domain)
...
## 3. 고급 데이터 증강 방법 (Advanced Methods)

### 3.1 분해 기반 (Decomposition-based)
...
### 3.2 통계적 생성 모델 (Statistical Generative Models)
...
## 5. 향후 연구 방향 (Future Opportunities)
...
## 6. 총평
이 논문은 시계열 데이터 증강 방법을 **도메인별·기법별·모델별로 체계적으로 분류한 첫 종합 리뷰**로서,  
시계열 데이터의 희소성과 불균형 문제를 다루는 연구자들에게 중요한 참조점이 됩니다.
...
전자상거래 수요예측(e-commerce demand forecasting)에서는
- **신규 점포/신상품의 데이터 부족**,
- **품절·이상값으로 인한 학습 불균형**,
- **짧은 시계열(window-based forecasting)**  
  등의 문제가 공통적으로 존재합니다.

이 논문에서 제시된 기법 중 다음이 특히 유용합니다:
...
> 🔍 **결론적으로**,  
> 시계열 데이터 증강은 “데이터 부족 문제를 해결하기 위한 전처리 기술”을 넘어,  
> **모델의 일반화·견고성·도메인 확장성**을 확보하는 핵심 연구축으로 진화하고 있습니다.  
> 수요예측에서도 Croston·TFT·XGBoost 등의 파이프라인에  
> 이러한 증강 전략을 체계적으로 결합한다면 cold-start 문제를 완화하고,  
> 예측 분산(uncertainty)을 줄이는 데 실질적 도움이 될 것입니다.
