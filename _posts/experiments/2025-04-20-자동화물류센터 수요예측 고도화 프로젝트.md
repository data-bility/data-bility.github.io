...
| 개선 방향 | 상세 계획                                                                                                                                                                                                                                   |
|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **블렌딩 로직 고도화** | 현재는 **TFT와 최종 선택된 비-TFT 모델의 단순 가중평균(0.5 : 0.5)** 구조로 결합하고 있습니다. <br>향후에는 SKU 특성(판매 빈도, 변동성)에 따라 가중치를 자동 조정하는 **adaptive blending** 방식을 검토할 예정입니다. <br>  (예시): `w_tft = f(trend_score)`, `w_selected = f(intermittency)` 형태의 동적 가중 함수 설계 |
| **해석 및 모니터링 자동화** | TFT의 Attention 가중치와 다른 모델의 Feature Importance를 통합 시각화하여, SKU별 예측 영향도를 정기적으로 추적할 수 있는 **모델 해석 대시보드**를 구축하는 것이 좋겠습니다. <br>(예시): Drift, Feature Shift 등을 주간 단위로 감지하는 ML monitoring 체계로 확장                                                  |
| **모델 선택 구조 개선** | Validation 기반 단일 선택에서 벗어나, 모델 간 불확실성(Uncertainty)과 일관성(Consistency)을 함께 고려한 **확률적 모델 선택(probabilistic selection)** 방식을 검토하고 있습니다. <br>(예시): 예측 분포의 신뢰구간(quantile)을 활용해 모델 간 confidence-weighted 선택 구조 구현 예정                             |
